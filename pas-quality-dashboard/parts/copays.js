...
(function () {
  ...
  function parseDate(v) {
    ...
    if (m) { var y = +m[3]; if (y < 100) y += 2000; return { y: y, mo: +m[1], d: +m[2] }; }
    m = s.match(/^(\d{4})[\/\-](\d{1,2})[\/\-](\d{1,2})/);
    if (m) return { y: +m[1], mo: +m[2], d: +m[3] };
    var d = new Date(s); if (!isNaN(d)) return { y: d.getFullYear(), mo: d.getMonth() + 1, d: d.getDate() };
    return null;
  }
  ...
  function copayWeekKey(y, mo, d, ws) {
    var dt = new Date(y, mo - 1, d);
    if (isNaN(dt.getTime())) return "";
    dt.setDate(dt.getDate() - ((dt.getDay() - ws + 7) % 7));
